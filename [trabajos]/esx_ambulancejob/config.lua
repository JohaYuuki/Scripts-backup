Config                            = {}

Config.DrawDistance               = 100.0

Config.Marker                     = { type = 1, x = 1.5, y = 1.5, z = 0.5, r = 102, g = 0, b = 102, a = 100, rotate = false }

Config.ReviveReward               = 100  -- revive reward, set to 0 if you don't want it enabled
Config.AntiCombatLog              = true -- enable anti-combat logging?
Config.LoadIpl                    = false -- disable if you're using fivem-ipl or other IPL loaders
Config.NPCJobEarnings             = {min = 100, max = 300}
Config.MinimumDistance            = 100 -- Minimum NPC job destination distance from the pickup in GTA units, a higher number prevents nearby destionations.

Config.Locale                     = 'fr'

local second = 1000
local minute = 60 * second

Config.EarlyRespawnTimer          = 4 * minute  -- Time til respawn is available
Config.BleedoutTimer              = 10 * minute -- Time til the player bleeds out

Config.EnablePlayerManagement     = true

Config.RemoveWeaponsAfterRPDeath  = true
Config.RemoveCashAfterRPDeath     = true
Config.RemoveItemsAfterRPDeath    = true

-- Let the player pay for respawning early, only if he can afford it.
Config.EarlyRespawnFine           = true
Config.EarlyRespawnFineAmount     = 500

Config.RespawnPoint = { coords = vector3(358.79, -589.58, 28.8), heading = 247.16 }

Config.Hospitals = {

	CentralLosSantos = {

		Blip = {
			coords = vector3(302.52, -585.56, 43.28), 
			sprite = 61,
			scale  = 1.0
		},

		AmbulanceActions = {
			vector3(298.84, -598.52, 42.28)	
		},

		Pharmacies = {
			vector3(309.65, -598.95, 42.28)
		},

		Vehicles = {
			{
				Spawner = vector3(297.52, -575.24, 43.12),
				InsideShop = vector3(362.49, -590.6, 28.67),
				Marker = { type = 36, x = 1.0, y = 1.0, z = 1.0, r = 100, g = 50, b = 200, a = 100, rotate = true },
				SpawnPoints = {
					{ coords = vector3(296.12, -606.64, 43.32), heading = 67.04, radius = 4.0 }

				}
			}
		},

		Helicopters = {
			{
				Spawner = vector3(354.83, -579.84, 74.17),
				InsideShop = vector3(351.88, -588.02, 74.17),
				Marker = { type = 34, x = 1.5, y = 1.5, z = 1.5, r = 100, g = 150, b = 150, a = 100, rotate = true },
				SpawnPoints = {
					{ coords = vector3(351.88, -588.02, 74.17), heading = 250.03, radius = 10.0 }
				}
			}
		}
	}
}

Config.AuthorizedVehicles = {

	ambulance = {
		{ model = 'ambulance', label = 'Ambulancia', price =1},
		{ model = 'dodgeEMS', label = 'Dodge EMS', price =1}
	},

	doctor = {
		{ model = 'ambulance', label = 'Ambulancia', price =1},
		{ model = 'dodgeEMS', label = 'Dodge EMS', price =1}
	},

	chief_doctor = {
		{ model = 'ambulance', label = 'Ambulancia', price =1},
		{ model = 'dodgeEMS', label = 'Dodge EMS', price =1}
	},

	boss = {
		{ model = 'ambulance', label = 'Ambulancia', price =1},
		{ model = 'dodgeEMS', label = 'Dodge EMS', price =1}
	}

}

Config.AuthorizedHelicopters = {

	ambulance = {
		{ model = 'uh1calfire', label = 'Helicoptero Rescate', price = 1 }
	},

	doctor = {
		{ model = 'uh1calfire', label = 'Helicoptero Rescate', price = 1 }
	},

	chief_doctor = {
		{ model = 'uh1calfire', label = 'Helicoptero Rescate', price = 1 }
	},

	boss = {
		{ model = 'uh1calfire', label = 'Helicoptero Rescate', price = 1 }
	}

}

Config.Uniforms = {
	[1] = { -- EMS
	  male = {
		['tshirt_1'] = 6, ['tshirt_2'] = 0, 
		['torso_1'] = 88,  ['torso_2'] = 0,
		['decals_1'] = 0,  ['decals_2'] = 0,
		['arms'] = 109,
		['pants_1'] = 51,   ['pants_2'] = 0,
		['shoes_1'] = 32,   ['shoes_2'] = 2,
		['helmet_1'] = -1, ['helmet_2'] = 0,
		['chain_1'] = 97,  ['chain_2'] = 0,
		['ears_1'] = 0,     ['ears_2'] = 0,
		['bproof_1'] = 0,   ['bproof_2'] = 0
	  },
	  female = {
		['tshirt_1'] = 6, ['tshirt_2'] = 0, 
		['torso_1'] = 88,  ['torso_2'] = 0,
		['decals_1'] = 0,  ['decals_2'] = 0,
		['arms'] = 109,
		['pants_1'] = 51,   ['pants_2'] = 0,
		['shoes_1'] = 32,   ['shoes_2'] = 2,
		['helmet_1'] = -1, ['helmet_2'] = 0,
		['chain_1'] = 97,  ['chain_2'] = 0,
		['ears_1'] = 0,     ['ears_2'] = 0,
		['bproof_1'] = 0,   ['bproof_2'] = 0
	  }
	},
	[2] = { -- medico
	  male = {
		['tshirt_1'] = 15,  ['tshirt_2'] = 0,
		['torso_1'] = 2,   ['torso_2'] = 0,
		['decals_1'] = 0,   ['decals_2'] = 0,
		['arms'] = 11,
		['pants_1'] = 1,   ['pants_2'] = 0,
		['shoes_1'] = 24,   ['shoes_2'] = 0,
		['helmet_1'] = -1,  ['helmet_2'] = 0,
		['chain_1'] = 0,    ['chain_2'] = 0,
		['ears_1'] = 0,     ['ears_2'] = 0,
		['bproof_1'] = 0,  ['bproof_2'] = 0
	  },
	  female = {
		['tshirt_1'] = 6,  ['tshirt_2'] = 0, 
		['torso_1'] = 197,   ['torso_2'] = 0,
		['decals_1'] = 0,   ['decals_2'] = 0,
		['arms'] = 109,
		['pants_1'] = 61,   ['pants_2'] = 8,
		['shoes_1'] = 25,   ['shoes_2'] = 0,
		['helmet_1'] = -1,  ['helmet_2'] = 0,
		['chain_1'] = 96,    ['chain_2'] = 0,
		['ears_1'] = 0,     ['ears_2'] = 0,
		['bproof_1'] = 0,  ['bproof_2'] = 0
	  }
	},
	[3] = { -- Cirujano
	  male = {
		['tshirt_1'] = 15,  ['tshirt_2'] = 0,
		['torso_1'] = 2,   ['torso_2'] = 0,
		['decals_1'] = 0,   ['decals_2'] = 0,
		['arms'] = 11,
		['pants_1'] = 1,   ['pants_2'] = 0,
		['shoes_1'] = 24,   ['shoes_2'] = 0,
		['helmet_1'] = -1,  ['helmet_2'] = 0,
		['chain_1'] = 0,    ['chain_2'] = 0,
		['ears_1'] = 0,     ['ears_2'] = 0,
		['bproof_1'] = 0,  ['bproof_2'] = 0
	  },
	  female = {
		['tshirt_1'] = 15,  ['tshirt_2'] = 0,
		['torso_1'] = 5,   ['torso_2'] = 0,
		['decals_1'] = 0,   ['decals_2'] = 0,
		['arms'] = 11,
		['pants_1'] = 3,   ['pants_2'] = 0,
		['shoes_1'] = 24,   ['shoes_2'] = 0,
		['helmet_1'] = -1,  ['helmet_2'] = 0,
		['chain_1'] = 0,    ['chain_2'] = 0,
		['ears_1'] = 0,     ['ears_2'] = 0,
		['bproof_1'] = 0,  ['bproof_2'] = 0
	  }
	},
	[4]= { -- Jefe
	  male = {
		['tshirt_1'] = 15,  ['tshirt_2'] = 0,
		['torso_1'] = 2,   ['torso_2'] = 0,
		['decals_1'] = 0,   ['decals_2'] = 0,
		['arms'] = 11,
		['pants_1'] = 1,   ['pants_2'] = 0,
		['shoes_1'] = 24,   ['shoes_2'] = 0,
		['helmet_1'] = -1,  ['helmet_2'] = 0,
		['chain_1'] = 0,    ['chain_2'] = 0,
		['ears_1'] = 0,     ['ears_2'] = 0,
		['bproof_1'] = 0,  ['bproof_2'] = 0
	  },
	  female = {
		['tshirt_1'] = 15,  ['tshirt_2'] = 0,
		['torso_1'] = 5,   ['torso_2'] = 0,
		['decals_1'] = 0,   ['decals_2'] = 0,
		['arms'] = 11,
		['pants_1'] = 3,   ['pants_2'] = 0,
		['shoes_1'] = 24,   ['shoes_2'] = 0,
		['helmet_1'] = -1,  ['helmet_2'] = 0,
		['chain_1'] = 0,    ['chain_2'] = 0,
		['ears_1'] = 0,     ['ears_2'] = 0,
		['bproof_1'] = 0,  ['bproof_2'] = 0
	  }
	}
}

Config.JobLocations = {
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67),
	vector3(362.3, -590.5, 28.67)
}
