				...
				if skin.sex == 0 then
					if Config.Uniforms[job].male then
						...
						if grade == 'recrue' then
							table.insert(elements, {label = _U('mechanic_wear'), value = 'recrue_wear'})
						elseif grade == 'novice' then
							table.insert(elements, {label = _U('mechanic_wear'), value = 'novice_wear'})
						elseif grade == 'experimente' then
							table.insert(elements, {label = _U('mechanic_wear'), value = 'experimente_wear'})
						elseif grade == 'chief' then
							table.insert(elements, {label = _U('mechanic_wear'), value = 'chief_wear'})
						elseif grade == 'boss' then
							table.insert(elements, {label = _U('mechanic_wear'), value = 'boss_wear'})
						end
					else
		...
		elseif data.current.value == 'boss_actions' then
			local options = {
			 ...
			 TriggerEvent('esx_society:openBossMenu', 'mechanic', function(data, menu)
				menu.close()
			--TriggerEvent('esx_society:openBosfamasMenu', 'mechanic', function(data, menu)
				--menu.close()
			end)
		end
	end, function(data, menu)
	...
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= 'recrue' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_craft', {
			...
			if data.current.value == 'fix_kit' then
				...
				TriggerServerEvent('esx_mechanicjob:starfamatCraft')
			end
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil or amount < 0 then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						menu.close()
						TriggerServerEvent('esx_billing:sendBill132', GetPlayerServerId(closestPlayer), 'society_mechanic', _U('mechanic'), amount)
					end
				end
			end, function(data, menu)
				menu.close()
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
				...
				elseif CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehiclefamaInGarage', 'mechanic', vehicleProps)

					else

						if
							GetEntityModel(vehicle) == GetHashKey('flatbed')   or
							GetEntityModel(vehicle) == GetHashKey('raptortow')
						then
