                ...
                if (Gotur[id].durum == 'iptal' or Gotur[id].durum == 'teslimedildi') then
                    ...
                    if (json.encode(test) ~= '[]') then
                        for i=1, #test, 1 do
                            ...
                            TriggerClientEvent('gksphone:notifi', test[i].id, {title = 'Yuki', message = Gotur[id].deneme .. _U('gotur_cancel'), img= '/html/static/img/icons/gotur.png' })
                        end
                    end
...
AddEventHandler('gks_gotur:siparis', function(deneme, a, b, c, d)
    ...
    if xPlayer ~= nil then
        ...
        if xPlayer.getAccount("bank").money > c then
                    ...
                    local test = getSourceFromIdentifier()
                     if (json.encode(test) ~= '[]') then
                        ...
                        for i=1, #test, 1 do
                            ...
                            TriggerClientEvent('gksphone:notifi', test[i].id, {title = 'Yuki', message = Gotur[id].deneme .._U('gotur_productbuy'), img= '/html/static/img/icons/gotur.png' })
                            TriggerClientEvent('gksphone:sipariss', test[i].id, Gotur)
                        end
            ...
            TriggerClientEvent('gksphone:notifi', src, {title = 'Yuki', message = _U('gotur_nomoney'), img= '/html/static/img/icons/gotur.png' })
        end
    end
end)
            ...
            TriggerEvent('esx_addonaccount:getSharedAccount', 'society_gotur', function(account)
                    ...
                    local tutar = Gotur[id].total
                        ...
                        for i=1, #test, 1 do
                            ...
                            TriggerClientEvent('gksphone:sipariss', test[i].id, Gotur)
                            TriggerClientEvent('gksphone:notifi', test[i].id, {title = 'Yuki', message = Gotur[id].deneme .._U('gotur_cancelorder'), img= '/html/static/img/icons/gotur.png' })
                        end
...
AddEventHandler('gks_gotur:syold', function(total, bilgi)
    ...
    if xPlayer ~= nil then
                    ...
                    local srck = getSourceFromIdentifiesr(iden)
                        ...
                        TriggerClientEvent('gksphone:notifi', srck, {title = 'Yuki', message = _U('gotur_orderway'), img= '/html/static/img/icons/gotur.png' })
...
AddEventHandler('gks_gotur:teslimyapildi', function(total, bilgi)
    ...
    if xPlayer ~= nil then
                    ...
                    local srck = getSourceFromIdentifiesr(iden)
                        ...
                        TriggerClientEvent('gksphone:notifi', srck, {title = 'Yuki', message = _U('gotur_orderdelivered'), img= '/html/static/img/icons/gotur.png' })
